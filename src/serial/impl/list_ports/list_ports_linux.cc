...
glob(const vector<string>& patterns)
{
    ...
    // Check for errors
    if (glob_retval == GLOB_NOMATCH) {
        std::cout << "No matches found for pattern: " << patterns[0] << std::endl;
        globfree(&glob_results);
        return paths_found;
    } else if (glob_retval != 0) {
        std::cerr << "Error with glob() function. Return code: " << glob_retval << std::endl;
        globfree(&glob_results);
        return paths_found;
    }
